    ...
    while(1)        // 主线程不断地循环检测监听描述符有无发生变化
    {
        ...
        for(int i = 0 ; i<num ; i++)        //遍历events事件数组，检测有无发生变化的文件描述符
        {
            ...
            if(cur_sockfd == listen_fd) //表示有客户端连接进来
            {
                ...
                users[connect_fd].init(connect_fd , client_address);  //5 将新客户的数据初始化，放到数组中 ，数组中保存每一个连接进来客户端的 文件描述符信息和地址信息
            }       
            ...
            {
                if( users[cur_sockfd].read())   //7 一次性把所有数据都读出来 proactor模式
                {
            ...
            else if(events[i].events & EPOLLOUT)   //表示检测到了写事件描述符发生变化 ，对读事件描述符进行读操作
            {
                if( !users[cur_sockfd].write())     //8 一次性写完所有数据
                {
                    users[cur_sockfd].close_conection();
                }
            }
